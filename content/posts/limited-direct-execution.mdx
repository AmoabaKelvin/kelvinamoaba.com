...
In operating systems we usually want to make two things happen. Whenever we are running programs, we want to always make sure the programs are running very fast but still giving the operating system the keys to the entire system. To make the programs run fast, what we do is to run its compiled code directly on the CPU without any further abstractions. Second, how does the OS ever claim the CPU back, because once something is running on the CPU, it basically means that the OS code is not running.
...
Earlier operating systems relied on a technique called `Cooperative Approach`. In this approach, the operating system expected to regain control when the process makes a system call and as a result, traps into the kernel. With the cooperative approach, there was a special system call which is the `yield` which was implemented so processes could call that in order to give the power back to the OS. But relying on a process to give away control, it's more or less like you not having control at all in the first place because assuming a process never does a system call or never calls yield and is in an infinite loop. How then do we go ahead to recover control for the CPU? That was the flaw in this approach, assuming a process never gave up the CPU (either through bugs or being a malware), how then would the system recover, the only option would be to reboot the system.

Thinking about this problem, operating system developers figured out that the best solution for this was to rely on the hardware to suspend programs after a couple of time in order to bring in the kernel code to be executed. This was achieved using the timer interrupts. During boot, a timer is configured to tick every x amount of time, and once that is done, the currently running process is paused and then control is given back to the kernel, the kernel would now have the chance to either continue making the already running program continue or might want to switch to another program through a context switch.

This approach relied on both the hardware and the software to work together to make it possible.
...
When the system boots up, the kernel programs the hardware timer chip to dictate how often an interrupt should occur. This is known as the tick rate. Before all of these can work well, the operating system would have to tell the CPU where to jump to when these interrupts occur. It writes its memory address into a specific slot reserved for the system clock in the trap table.
